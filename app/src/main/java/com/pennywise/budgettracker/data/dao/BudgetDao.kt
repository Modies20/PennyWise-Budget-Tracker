...
interface BudgetDao {
    ...
    @Query("""
        SELECT * FROM budgets 
        WHERE userId = :userId AND month = :month AND year = :year AND categoryId IS NULL
    """)
    suspend fun getTotalMonthlyBudget(userId: Long, month: Int, year: Int): Budget?

    @Query("""
        SELECT * FROM budgets 
        WHERE userId = :userId AND month = :month AND year = :year AND categoryId = :categoryId
    """)
    ...
    suspend fun deleteBudgetsForMonth(userId: Long, month: Int, year: Int)
}