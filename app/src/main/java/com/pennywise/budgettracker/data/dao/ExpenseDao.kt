...
interface ExpenseDao {
    ...
    suspend fun getTotalSpentByCategory(userId: Long, categoryId: Long, startDate: Long, endDate: Long): Double?

    @Query("SELECT SUM(amount) FROM expenses WHERE userId = :userId AND date BETWEEN :startDate AND :endDate")
    suspend fun getTotalSpentBetweenDates(userId: Long, startDate: Long, endDate: Long): Double?
